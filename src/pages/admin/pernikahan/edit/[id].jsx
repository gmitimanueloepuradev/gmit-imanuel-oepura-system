...
export default function EditPernikahanPage() {
  ...
  // Fetch ALL jemaat data (yang belum menikah + yang sudah terpilih di pernikahan ini)
  ...
  useEffect(() => {
    if (pernikahanData?.data) {
      ...
      const tanggalFormatted = new Date(pernikahan.tanggal)
        .toISOString()
        .split("T")[0];

      form.reset({
        tanggal: tanggalFormatted,
        idKlasis: pernikahan.idKlasis,
        jemaatIds: pernikahan.jemaats?.map((j) => j.id) || [],
      });

      setSelectedJemaats(pernikahan.jemaats || []);
    }
  }, [pernikahanData, form]);

  // Update mutation
  const updateMutation = useMutation({
    mutationFn: (data) => pernikahanService.update(id, data),
    onSuccess: () => {
      showToast({
        title: "Berhasil",
  ...
  // Filter jemaat yang belum menikah + yang sudah terpilih di pernikahan ini
  ...
  if (!pernikahan) {
    return (
      <div className="text-center py-8">
        <p className="text-gray-500">Data pernikahan tidak ditemukan</p>
        <Button
          className="mt-4"
          variant="outline"
    ...
    <>
      <PageHeader
        ...
        title={`Edit Pernikahan - ${
          pernikahan.jemaats?.map((j) => j.nama).join(" & ") ||
          "Data Pernikahan"
        }`}
      />
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <form className="space-y-6" onSubmit={form.handleSubmit(handleSubmit)}>
          ...
          <div className="flex justify-end space-x-3">
            ...
            <Button
              disabled={
                updateMutation.isLoading || selectedJemaats.length === 0
              }
              type="submit"
            >
