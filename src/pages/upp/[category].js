    ...
    return (
      <div className="flex justify-center items-center h-screen">
        <div className="text-center">
          ...
          <p className="text-gray-600">The requested UPP category could not be found.</p>
        </div>
      </div>
    );
  }

  const hasSubcategories = categoryData.jenisPengumuman && categoryData.jenisPengumuman.length > 0;
  ...
  return (
    <div>
      <div className="flex justify-center items-center h-screen">
        ...
        />
        <h1 className="absolute text-6xl md:text-8xl text-white font-bold mt-4">UPP {categoryData.nama}</h1>
      </div>
      ...
      {hasSubcategories ? (
        <div className="min-h-screen bg-gray-100 p-8">
          <div className="max-w-6xl mx-auto">
            <h2 className="text-3xl font-bold text-gray-800 mb-8 text-center">Pilih Kategori {categoryData.nama}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {categoryData.jenisPengumuman.map((jenis) => (
                ...
                >
                  <h3 className="text-xl font-semibold text-gray-800 mb-2">{jenis.nama}</h3>
                  {jenis.deskripsi && <p className="text-gray-600 text-sm">{jenis.deskripsi}</p>}
                </Link>
              ))}
            </div>
          </div>
