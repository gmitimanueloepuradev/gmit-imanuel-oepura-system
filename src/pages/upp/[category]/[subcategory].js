...
export default function UppSubcategory() {
  ...
  const { getCategoryBySlug, getSubcategoryBySlug, loading } = useKategoriPengumuman();
    ...
    return (
      <div className="flex justify-center items-center h-screen">
        <div className="text-center">
          ...
          <p className="text-gray-600">The requested UPP category or subcategory could not be found.</p>
        </div>
      </div>
    );
  }
    ...
    <div>
      <div className="flex justify-center items-center h-screen">
        ...
        <div className="absolute text-center">
          <h1 className="text-6xl md:text-8xl text-white font-bold">UPP {categoryData.nama}</h1>
          <h2 className="text-3xl md:text-4xl text-white font-semibold mt-4">{subcategoryData.nama}</h2>
        </div>
      </div>
